...
import { 
  ...
  Copy, Trash2, ChevronDown, ChevronRight, FileText, CheckSquare, 
  ToggleLeft, AlignLeft, HelpCircle, Sparkles, Filter, Grid
} from 'lucide-react';
...
export const QuestionOrganizer: React.FC = () => {
  const { 
    ...
    moveQuestionUp, moveQuestionDown 
  } = useEditorStore();
  ...
  const filteredQuestions = allQuestions.filter(({ element }) => {
    ...
    const cleanText = element.content.replace(/^س\d+:\s*/, '');
  ...
  if (isPanelCollapsed) {
    return (
      <div className="bg-slate-900 border-l border-slate-800 p-2 flex flex-col items-center gap-3 shrink-0 z-20 select-none">
        <button 
    ...
    <aside className="w-72 md:w-80 bg-slate-900 border-l border-slate-800 text-slate-200 flex flex-col shrink-0 z-20 select-none h-full font-sans" dir="rtl">
      ...
      <div className="p-3 border-b border-slate-800 flex items-center justify-between gap-2 bg-slate-950/50">
        <div className="flex items-center gap-2 text-indigo-400 font-black text-sm">
          ...
          <span>منظم الأسئلة (Organizer)</span>
        </div>
        <button 
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5 custom-scrollbar">
        {filteredQuestions.length === 0 ? (
          ...
          filteredQuestions.map(({ element, pageIndex }) => {
            ...
            const cleanText = element.content.replace(/^س\d+:\s*/, '').replace(/\s*\[\d+\s*درجات\]$/, '');
